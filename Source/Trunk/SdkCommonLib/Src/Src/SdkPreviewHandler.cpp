...
SdkPreviewHandler::SdkPreviewHandler() : m_pHandler(NULL),
...
SdkPreviewHandler::~SdkPreviewHandler()
{
    DetachPreview();
...
BOOL SdkPreviewHandler::AttachPreview(IN HWND hWnd, IN const RECT* lpRect, IN LPCWSTR lpfileName)
{
...
void SdkPreviewHandler::DetachPreview()
{
    if ( NULL != m_pHandler )
    {
        m_pHandler->Unload();
...
void SdkPreviewHandler::EnumInstalledHandler()
{
        ...
        if ( ERROR_SUCCESS == lResult )
        {
            ...
            for (int i = 0; i < (int)dwHandlersCount; ++i)
            {
                ...
                lResult = RegEnumValue(
                    ...
                    NULL,
                    NULL,
                    (LPBYTE)handlerInfo->szName, &dwSize);
...
DWORD SdkPreviewHandler::GetInstalledHandlerCount()
{
...
BOOL SdkPreviewHandler::GetInstalledHandlerItem(IN DWORD nIndex, OUT LPPREVIEWHANDLERINFO previewHandleInfo)
{
...
BOOL SdkPreviewHandler::GetClsidFromExtension(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
    ...
    if ( bSuccess )
    {
        m_mapExtensionToCLSID.insert(
            make_pair(SdkCommonHelper::StringToLowerCase(lpfileExt), wstring(pszClsid)));
    }
...
BOOL SdkPreviewHandler::CreatePreviewHandlerFromClsid(OUT IPreviewHandler** ppHandler, IN LPCTSTR pClsid)
{
...
BOOL SdkPreviewHandler::InitializeHandler(IN IPreviewHandler* pHandler, IN LPCWSTR lpfileName)
{
    if (NULL != lpfileName)
    {
...
BOOL SdkPreviewHandler::InitializeHandlerWithFile(IN IPreviewHandler* pHandler)
{
    if ( NULL == pHandler )
    {
        return FALSE;
...
BOOL SdkPreviewHandler::InitializeHandlerWithStream(IN IPreviewHandler* pHandler)
{
    if ( NULL == pHandler )
    {
        return FALSE;
    ...
    if ( ISVALIDHANDLE(hFile) )
    {
        ...
        {
            HGLOBAL hGlobalMemory = GlobalAlloc(GMEM_FIXED, dwFileSize);
            BYTE *pData = (BYTE*)GlobalLock(hGlobalMemory);
            if ( NULL != pData )
            {
                DWORD dwRead = 0;
                ReadFile(hFile, pData, dwFileSize, &dwRead, NULL);
                IStream *pStream = NULL;
                hr = CreateStreamOnHGlobal(hGlobalMemory, TRUE, &pStream);
...
BOOL SdkPreviewHandler::InitializeHandlerWithItem(IN IPreviewHandler* pHandler)
{
    if ( NULL == pHandler )
    {
        return FALSE;
...
BOOL SdkPreviewHandler::GetClsidFromMap(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
    if ( (NULL != pszClsid) && (dwSize > 0) )
    {
        map<wstring, wstring>::iterator item = 
            m_mapExtensionToCLSID.find(SdkCommonHelper::StringToLowerCase(lpfileExt));
...
BOOL SdkPreviewHandler::GetClsidFromDefaultType(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
...
BOOL SdkPreviewHandler::GetClsidFromExtShellEx(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
    if ( (NULL == lpfileExt) || (NULL == pszClsid) || (dwSize <= 0) )
    {
        return FALSE;
...
BOOL SdkPreviewHandler::GetClsidFromSystemFileAssoc(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
    if ( (NULL == lpfileExt) || (NULL == pszClsid) || (dwSize <= 0) )
    {
        return FALSE;
    ...
    LONG lResult = -1;
    HKEY hFileExtKey = NULL;
    lResult = RegOpenKeyEx(HKEY_CLASSES_ROOT, lpfileExt, 0, KEY_QUERY_VALUE, &hFileExtKey);
    if ( ERROR_SUCCESS != lResult )
    {
        return FALSE;
    }
    ...
    if ( ERROR_SUCCESS == lResult )
    {
        ...
        StringCchPrintf(
            szSubKeyText,
            MAX_PATH,
            L"SystemFileAssociations\\%s\\ShellEx\\%s",
            szPerceivedText,
            g_pPreviewHandlerQueryGUID);
...
BOOL SdkPreviewHandler::GetClsidFromSystemFileAssocExt(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
    if ( (NULL == lpfileExt) || (NULL == pszClsid) || (dwSize <= 0) )
    {
        return FALSE;
...
BOOL SdkPreviewHandler::GetClsidFromOpenWithProgIds(IN LPCWSTR lpfileExt, OUT LPTSTR pszClsid, IN DWORD dwSize)
{
    if ( (NULL == lpfileExt) || (NULL == pszClsid) || (dwSize <= 0) )
    {
        return FALSE;
...
void SdkPreviewHandler::ClearInstalledHandlerList()
{
