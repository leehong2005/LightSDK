...
void D2DTextLayout::SetText(IN LPCTSTR lpText)
{
    if ( NULL != lpText )
    {
        SAFE_DELETE_ARRAY(m_pText);
        ...
        m_pText = new TCHAR[length];
        ZeroMemory(m_pText, length * sizeof(TCHAR));
...
void D2DTextLayout::GetText(IN LPTSTR lpszText, UINT32 uSize)
{
...
void D2DTextLayout::SetTextAlignment(IN DWRITE_TEXT_ALIGNMENT textAlign)
{
    ...
    {
        if ( textAlign != m_pDWriteTextFormat->GetTextAlignment() )
        {
...
void D2DTextLayout::ReCreateTextLayout()
{
...
void D2DTextLayout::GetIDWriteTextLayout(OUT IDWriteTextLayout **ppDWriteTextLayout)
{
    ...
    {
        // Before get the DriectWrite text layout, re-create a new text layout, because
        // properties of text layout may have changed before calling this method.
        ReCreateTextLayout();

        (*ppDWriteTextLayout) = m_pDWriteTextLayout;
        SAFE_ADDREF((*ppDWriteTextLayout));
    }
}
...
void D2DTextLayout::DrawText(IN ID2D1RenderTarget *pRenderTarget)
{
    if ( (NULL != pRenderTarget) && (NULL != m_pDWriteTextLayout) )
    {
        ...
        // Re-create text layout, because the properties of text layout may have changed,
        // in this method, we have a flag to indicate should re-create or not.
        ReCreateTextLayout();
        ...
        {
            // Use the custom text renderer.
            m_pTextRender = new D2DCustomTextRenderer(pD2DFactory, pRenderTarget, pD2DBrush);
        }
