...
BOOL D3DDevice::InitDevice( HWND hWnd )
{
    ...
    if (!IsWindow( hWnd ))
    {
        SdkCommonHelper::PrintDebugString(L"Error: not valid window hwnd!");
        return FALSE;
    }
    ...
    if (!(m_pD3D)) 
    {
        SdkCommonHelper::PrintDebugString(L"Error: Can not create Direct9!");
        exit(0);
    }
        ...
        &m_pd3dDevice))) 
    {
            ...
            &m_pd3dDevice))) 
        {
            ...
            if (FAILED(hr = m_pD3D->CreateDevice(D3DADAPTER_DEFAULT, 
                ...
                &m_pd3dDevice))) 
            {
                SdkCommonHelper::PrintDebugString(L"Can not create 3D device %x\n", hr);
                SetLastErrorEx(hr,0);
                return FALSE;
            }
        }
...
BOOL D3DDevice::ResetDevice()
{
    if ( m_pd3dDevice != NULL )
    {
        ...
        SdkCommonHelper::PrintDebugString(L"d3device reset 0x%x\n", hr);
    }
    return FALSE;
}
