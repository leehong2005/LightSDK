...
{
    m_brushColor = D2D1::ColorF(D2D1::ColorF::White);
}
...
BOOL D2DSolidColorBrush::CreateBrush(IN ID2D1RenderTarget *pRenderTarget)
{
    ...
    {
        // Set the notification to the Device, so that the brush can get notification 
        ...
        if ( SUCCEEDED(hr) )
        {
            m_pSolidColorBrush->SetColor(m_brushColor);
        }
