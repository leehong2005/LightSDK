/*!
* @file QueryUrlHistory.cpp
* 
* @brief This file defines QueryUrlHistory class to query url history of internet explorer.
* 
...
#include "QueryUrlHistory.h"
#include "CommonHelper.h"
#include <algorithm>

USING_NAMESPACE_COMMON

QueryUrlHistory::QueryUrlHistory() : m_pUrlHistoryStg2(NULL),
                                     m_pEnumStatUrl(NULL),
                                     m_uCursorIndex(0)
{
    Initialize();
...
QueryUrlHistory::~QueryUrlHistory()
{
    ClearSearchUrls();
...
BOOL QueryUrlHistory::AddUrl(LPCOLESTR pocsUrl, LPCOLESTR pocsTitle)
{
    HRESULT hr = E_FAIL;
...
BOOL QueryUrlHistory::DeleteUrl(LPCOLESTR pocsUrl)
{
    HRESULT hr = E_FAIL;
    if (NULL != m_pUrlHistoryStg2)
    {
...
BOOL QueryUrlHistory::Next(OUT STATURL &statUrl)
{
...
BOOL QueryUrlHistory::Initialize()
{
...
void QueryUrlHistory::EnumUrlHistory(UINT uUrlsNum)
{
    ...
    while (SUCCEEDED(m_pEnumStatUrl->Next(1, &statUrl, &celtFetched)) && (1 == celtFetched))
    {
        if (m_vctSearchUrls.size() < uUrlsNum)
        {
            if (IsValidSTATURL(&statUrl))
            {
                ...
                LONG lRes = CompareFileTime(&(statUrl.ftLastVisited), &(m_vctSearchUrls[uMinIndex].ftLastVisited));
                if (lRes < 0)
                {
...
void QueryUrlHistory::ClearSearchUrls()
{
...
UINT QueryUrlHistory::FindEarliestUrl()
{
    vector<STATURL>::iterator item = min_element(m_vctSearchUrls.begin(), m_vctSearchUrls.end(), STATURLLesser);
...
BOOL QueryUrlHistory::IsValidSTATURL(const STATURL *pstatUrl)
{
    if (NULL == pstatUrl)
    {
        return FALSE;
...
BOOL QueryUrlHistory::STATURLLesser(const STATURL &firstItem, const STATURL &secondItem)
{
    LONG lRes = CompareFileTime(&(firstItem.ftLastVisited), &(secondItem.ftLastVisited));
    return (lRes < 0) ? TRUE : FALSE;
}

//////////////////////////////////////////////////////////////////////////

BOOL QueryUrlHistory::STATURLGreater(const STATURL &firstItem, const STATURL &secondItem)
{
    LONG lRes = CompareFileTime(&(firstItem.ftLastVisited), &(secondItem.ftLastVisited));
    return (lRes > 0) ? TRUE : FALSE;
}
