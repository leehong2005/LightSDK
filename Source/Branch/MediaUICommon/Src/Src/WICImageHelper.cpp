...
IWICImagingFactory* WICImageHelper::s_pImagingFactory = NULL;

HRESULT WICImageHelper::WICInitialize()
{
...
void WICImageHelper::WICUninitialize()
{
    SAFE_RELEASE(s_pImagingFactory);
}
...
BOOL WICImageHelper::SaveWICBitmapToFile(PCWSTR pFileName, IWICBitmapSource *pWICBitmap)
{
...
HBITMAP WICImageHelper::CreateHBITMAPFromIWICBitmap(HDC hDC, IWICBitmapSource *pWICBitmapSource)
{
    UINT uWidth = 0;
    UINT uHeight = 0;
...
WICImageHelper::WICImageHelper() : m_curImageType(WIC_IMAGE_TYPE_PNG),
...
BOOL WICImageHelper::LoadFromFile(LPCWSTR lpfile, UINT32 uDestWidth, UINT32 uDestHeight)
{
    if (NULL == s_pImagingFactory)
    {
        return FALSE;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                IWICBitmapSource *pSource = (NULL != pScaler) ? ((IWICBitmapSource*)pScaler) : ((IWICBitmapSource*)pTempSource);
                if (NULL == pSource)
                {
                    pSource = pFrame;
                }
...
BOOL WICImageHelper::LoadFromHBITMAP(HBITMAP hBitmap, UINT32 uDestWidth, UINT32 uDestHeight)
{
    if (NULL == s_pImagingFactory)
    {
        return FALSE;
    ...
    SAFE_RELEASE(m_pConvertedSourceBitmap);
    HRESULT hr = s_pImagingFactory->CreateBitmapFromHBITMAP(hBitmap, NULL, WICBitmapUseAlpha, &ppIBitmap);
    if (SUCCEEDED(hr))
    {
        hr = s_pImagingFactory->CreateFormatConverter(&m_pConvertedSourceBitmap);
        if (SUCCEEDED(hr))
        {
            IWICBitmapScaler *pScaler = CreateBitmapScaler(ppIBitmap, uDestWidth, uDestHeight);
            IWICBitmapSource *pSource = (NULL != pScaler) ? ((IWICBitmapSource*)pScaler) : ((IWICBitmapSource*)ppIBitmap);
...
BOOL WICImageHelper::LoadFromResource(UINT uResId, HMODULE hModule, UINT32 uDestWidth, UINT32 uDestHeight)
{
    if (NULL == s_pImagingFactory)
    {
        return FALSE;
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        IWICBitmapScaler *pScaler = CreateBitmapScaler(pFrame, uDestWidth, uDestHeight);
                        IWICBitmapSource *pSource = (NULL != pScaler) ? ((IWICBitmapSource*)pScaler) : ((IWICBitmapSource*)pFrame);
...
BOOL WICImageHelper::GetFormatConverter(IWICFormatConverter **ppConvertedSourceBitmap)
{
...
void WICImageHelper::SetImageType(WIC_IMAGE_TYPE type)
{
    m_curImageType = type;
}
...
HRESULT WICImageHelper::CreateScalerWICBitmapSource(UINT32 uWidth, UINT32 uHeight)
{
...
HRESULT WICImageHelper::CloneWICBitmapSource(IWICBitmapSource *pWICBitmapSource, IWICBitmapSource **ppWICBitmapDest)
{
...
void WICImageHelper::GetImageInfos()
{
    GetWidth();
    GetHeight();
}
...
DWORD WICImageHelper::LoadResourceData(UINT uResId, HMODULE hModule, OUT PVOID* pData)
{
...
IWICBitmapScaler* WICImageHelper::CreateBitmapScaler(IN IWICBitmapSource* pWICBitmapSource, UINT32 uDestWidth, UINT32 uDestHeight)
{
