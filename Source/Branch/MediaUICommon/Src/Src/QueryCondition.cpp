...
QueryCondition::QueryCondition() : m_pQueryParser(NULL),
                                   m_pszQuerys(NULL),
                                   m_nQueryKind(0),
                                   m_uQueryCount(0)
{
...
HRESULT QueryCondition::GetCondition(ICondition **ppc)
{
    if (NULL == ppc)
    {
        return E_INVALIDARG;
    ...
    *ppc = NULL;
    ...
    if (conditionCount > 0)
    {
        ...
        hr = pConditionFactory->CreateCompoundFromArray(CT_OR_CONDITION, pszConditions, conditionCount, CONDITION_CREATION_DEFAULT, IID_PPV_ARGS(ppc));
...
HRESULT QueryCondition::SetCondition(LPCWSTR *ppszQuerys, UINT32 uCount)
{
...
HRESULT QueryCondition::SetCondition(INT32 queryKind)
{
...
HRESULT QueryCondition::ClearCondtions()
{
    if (NULL != m_pszQuerys)
    {
        for (UINT32 i = 0; i < m_uQueryCount; ++i)
...
HRESULT QueryCondition::CreateQueryParser()
{
    SAFE_RELEASE(m_pQueryParser);
    IQueryParserManager *pQueryParserManager = NULL;

    HRESULT hr = CoCreateInstance(CLSID_QueryParserManager, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pQueryParserManager));
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            for (int i = 0; i < ARRAYSIZE(g_rgGenericProperties) && SUCCEEDED(hr); ++i)
            {
                ...
                hr = pQueryParser->SetMultiOption(SQMO_DEFAULT_PROPERTY, pszSemanticType, &propVar);
...
HRESULT QueryCondition::ParseStructuredQuery(QUERYCONDITIONKIND kind, OUT ICondition **ppc)
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = pConditionFactory->CreateStringLeaf(PKEY_Kind, COP_EQUAL, strKind, NULL, CONDITION_CREATION_DEFAULT, IID_PPV_ARGS(ppc));
    }
...
HRESULT QueryCondition::ParseStructuredQuery(PCWSTR pszString, OUT ICondition **ppc)
{
...
HRESULT QueryCondition::GetQueryKindList(OUT vector<QUERYCONDITIONKIND> &vctKinds)
{
