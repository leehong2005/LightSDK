...
QueryScope::TYPETOFOLDERIDMAP QueryScope::s_mapTypeToFolderID;

void QueryScope::InitializeMap()
{
    ...
    {
        s_mapTypeToFolderID.insert(MAKE_PAIR(g_szKeyValuePairs[i].type, g_szKeyValuePairs[i].folderId));
    }
}

//////////////////////////////////////////////////////////////////////////

QueryScope::QueryScope() : m_nScopeType(0),
...
HRESULT QueryScope::SetScope(INT32 scopeType)
{
...
HRESULT QueryScope::SetScope(PCWSTR *pszScopePaths, UINT32 uCount)
{
...
HRESULT QueryScope::GetScope(OUT IShellItemArray **ppsia)
{
    if (NULL == ppsia)
    {
        return E_INVALIDARG;
    ...
    for (UINT32 i = 0; i < m_uScopePathsCount; ++i)
    {
        ...
        hr = SHCreateItemFromParsingName(m_pszScopePaths[i], NULL, IID_PPV_ARGS(&pShellItem2));
        if (SUCCEEDED(hr))
        {
            PIDLIST_ABSOLUTE pItemID = NULL;
...
HRESULT QueryScope::GetScopeType(OUT vector<QUERYSCOPETYPE> &vctScopeTypes)
{
