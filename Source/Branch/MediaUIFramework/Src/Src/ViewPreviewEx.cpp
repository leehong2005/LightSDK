...
ViewPreviewEx::ViewPreviewEx() : m_fRealWidth(0),
                                 m_fOffsetX(0),
                                 m_fBackOffsetX(0),
                                 m_fZoomRate(1),
...
void ViewPreviewEx::SetEelementArea(FLOAT fWidth, FLOAT fHeight)
{
...
BOOL ViewPreviewEx::AddView(IN const ViewElement *pChild)
{
    ViewLayout *pLayout = new ViewLayout();
    m_vecFrame.push_back(pLayout);
    m_vecContent.push_back(const_cast<ViewElement *>(pChild));
    pLayout->AddView(pChild);
    return ViewLayout::AddView(pLayout);
}

//////////////////////////////////////////////////////////////////////////

BOOL ViewPreviewEx::RemoveAllChildren(BOOL isClearCache)
{
    this->StopAnimation();
    ...
    return ViewLayout::RemoveAllChildren();
}

//////////////////////////////////////////////////////////////////////////

void ViewPreviewEx::SetCanZoom(BOOL bCanZoom)
{
    m_bCanZoom = bCanZoom;
    m_bMove = FALSE;
    if(!m_bCanZoom)
...
BOOL ViewPreviewEx::OnMouseEvent(UINT message, WPARAM wParam, LPARAM lParam)
{
...
BOOL ViewPreviewEx::OnTouchEvent(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void ViewPreviewEx::OnLayout(BOOL bChanged, FLOAT left, FLOAT top, FLOAT width, FLOAT height)
{
...
void ViewPreviewEx::OnLButtonDown(WPARAM wParam, LPARAM lParam)
{
    UNREFERENCED_PARAMETER(wParam);
...
void ViewPreviewEx::OnLButtonUp(WPARAM wParam, LPARAM lParam)
{
    UNREFERENCED_PARAMETER(wParam);
...
void ViewPreviewEx::OnMouseMove(WPARAM wParam, LPARAM lParam)
{
    UNREFERENCED_PARAMETER(wParam);
...
void ViewPreviewEx::DoPanning(const POINT &pointLast, const POINT &pointCur)
{
    if(m_bCanPanning)
    {
        if(m_nCurShowIndex >= 0 && m_nCurShowIndex < (INT32)m_vecContent.size())
        {
            ViewElement *pView = m_vecContent[m_nCurShowIndex];
            LAYOUTINFO layout = {0};
...
void ViewPreviewEx::OnMouseLeave( WPARAM wParam, LPARAM lParam )
{
    this->OnLButtonUp(wParam, lParam);
}
...
void ViewPreviewEx::OnMouseWheel(WPARAM wParam, LPARAM lParam)
{
...
FLOAT ViewPreviewEx::CalcOffset()
{
...
void ViewPreviewEx::CalcOffset(FLOAT &fStartOffset, FLOAT &fEndOffset)
{
...
void ViewPreviewEx::SetFitChildToWindow(BOOL bFit)
{
...
void ViewPreviewEx::SetStartChild(INT32 nChildIndex)
{
...
void ViewPreviewEx::ScrollUp(BOOL bNeedInvalidate)
{
...
void ViewPreviewEx::ScrollDown(BOOL bNeedInvalidate)
{
    switch(m_animationType)
    {
    case ANIMATION_UNIFORM:
...
void ViewPreviewEx::CorrectOffset(BOOL bNeedInvalidate)
{
	...
	if(m_bZoom)
    {
        if(m_nCurShowIndex >= 0 && m_nCurShowIndex < (INT32)m_vecContent.size())
        {
            m_vecContent[m_nCurShowIndex]->SetLayoutInfo(&m_curChildLayoutInfo);
        }
        m_fZoomRate = 1;
        m_bZoom = FALSE;
        m_bCanPanning = FALSE;
...
void ViewPreviewEx::PerformSwitch()
{
...
void ViewPreviewEx::SetSwitchEventHandler(IN ISwitchEventHandler* pEventHandler)
{
...
INT32 ViewPreviewEx::GetCurrentIndex()
{
...
void ViewPreviewEx::StartAnimation(IN BOOL isStart, IN BOOL bLeft)
{
    if (isStart)
    {
        m_isAnimationEnd = FALSE;
...
void ViewPreviewEx::OnAnimationStart( Animation* pAnimation)
{
...
void ViewPreviewEx::OnAnimationPlaying(Animation* pAnimation)
{
    UNREFERENCED_PARAMETER(pAnimation);
}
...
void ViewPreviewEx::OnAnimationEnd(Animation* pAnimation)
{
    UNREFERENCED_PARAMETER(pAnimation);
...
VOID ViewPreviewEx::OnAnimationTimerUpdate(Animation *pAnimation)
{
    if (NULL != pAnimation)
    {
        DOUBLE dDoubleX = 0;
...
void ViewPreviewEx::SetAnimationType(IN ANI_TYPE type)
{
...
void ViewPreviewEx::SetAnimationTime(IN DOUBLE nDSecond)
{
...
BOOL ViewPreviewEx::IsAnimationEnd()
{
...
void ViewPreviewEx::ResizeFrame(FLOAT fParentWidth, FLOAT fParentHeight)
{
    ...
    for(INT32 nIndex = 0; nIndex < nCount; ++nIndex)
    {
        ...
        {
            m_vecFrame[nIndex]->SetLayoutInfo(&layoutInfo);
            if(m_bFitChildToWindow)
            {
            ...
            m_vecContent[nIndex]->SetLayoutInfo(&layoutInfoContent);
...
void ViewPreviewEx::ZoomIn(FLOAT fRate, POINT *pPointCenter)
{
...
void ViewPreviewEx::ZoomOut(FLOAT fRate, POINT *pPointCenter)
{
...
void ViewPreviewEx::Zoom(FLOAT fRate, POINT *pPointCenter)
{
    ...
    if(m_nCurShowIndex >= 0 && m_nCurShowIndex < nCount)
    {
        ...
        if(NULL != pView)
        {
            ...
            pView->SetLayoutInfo(&layoutInfo);
            pView->Invalidate();
        }
    }
}
...
void ViewPreviewEx::ZoomTo(FLOAT fRate , POINT *pPointCenter)
{
    if(fRate < MIN_ZOOM)
    {
        fRate = MIN_ZOOM;
...
void ViewPreviewEx::CancelZoom()
{
    if(m_bZoom)
    {
        ...
        if(m_nCurShowIndex >= 0 && m_nCurShowIndex < nCount)
        {
            ...
            if(NULL != pView)
            {
                pView->SetLayoutInfo(&m_curChildLayoutInfo);
            }
        }
...
void ViewPreviewEx::UpdateCurShowIndex()
{
...
void ViewPreviewEx::SetGesture(BOOL bInteria)
{
...
void ViewPreviewEx::StopAnimation()
{
...
void ViewPreviewEx::StopAnimation(Animation *pAnimation)
{
    if(NULL != pAnimation)
    {
        pAnimation->Stop();
