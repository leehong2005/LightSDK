...
HRESULT D2DDevice::InitDevice(HWND hWnd)
{
    HRESULT hr = S_OK;
...
HRESULT D2DDevice::InitMemDevice(HDC hdc)
{
    ...
    if (NULL == m_pWICBitmapRenderTarget)
    {
        ...
        hr = CreateWICBitmapRenderTarget(hdc, m_hMemDC, &m_pWICBitmapRenderTarget, &m_pWICBitmap);
    }
...
void D2DDevice::GetD2DFactory(OUT ID2D1Factory **ppD2DFactory)
{
    if (NULL == m_pD2DFactory)
    {
...
{
    HBITMAP hBitmap = WICImageHelper::CreateHBITMAPFromIWICBitmap(m_hMemDC, m_pWICBitmap);
...
void D2DDevice::BeginDraw(HDC hDC, const LPRECT lpRect, BOOL isClearRT)
{
    ...
    {
    case DEVICE_TARGET_TYPE_HWND:
        if ( SUCCEEDED(InitDevice(m_hWnd)) && (NULL != m_pRenderTarget) )
        {
...
{
    if ( (DEVICE_TARGET_TYPE_HWND == m_paintTargetType) && (NULL != m_pRenderTarget) )
    {
...
HRESULT D2DDevice::CreateWICBitmapRenderTarget(IN HDC hDC, OUT HDC& hOutMemDC, OUT ID2D1RenderTarget **ppTarget, OUT IWICBitmap **ppWICBitmap)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = pWICFactory->CreateBitmap(uWidth, uHeight, GUID_WICPixelFormat32bppPBGRA, WICBitmapCacheOnLoad, &pWICBitmap);
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = pDWriteGdiInterop->CreateBitmapRenderTarget(hDC, uWidth, uHeight, &m_pDWriteBitmapTarget);
            if (SUCCEEDED(hr))
            {
