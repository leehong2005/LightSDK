...
{
    if (pt.x > srcRc.left && pt.x < srcRc.right && pt.y > srcRc.top && pt.y < srcRc.bottom)
    {
        return TRUE;
    }
...
{
    BOOL isEqual = ( ((LONG)rc1.left == (LONG)rc2.left) && ((LONG)rc1.top == (LONG)rc2.top) &&
                     ((LONG)rc1.right == (LONG)rc2.right) && ((LONG)rc1.bottom == (LONG)rc2.bottom) );
