...
D2DAnimatedGif::D2DAnimatedGif() : m_pD2DBitmap(NULL),
                                   ...
                                   m_pWicAnimatedGif(new WICAnimatedGif())
{
}
...
BOOL D2DAnimatedGif::OnPaintNextFrame()
{
    if (NULL == m_pBitmapRenderTarget)
    {
        ...
        if (NULL != pD2DDevice)
        {
            ...
            HRESULT hr = pTarget->CreateCompatibleRenderTarget(D2D1::SizeF((FLOAT)uWidth, (FLOAT)uHeight), &m_pBitmapRenderTarget);

            if (FAILED(hr))
            {
                D2DUtility::CreateScalerRect(uWidth, uHeight);

                hr = pTarget->CreateCompatibleRenderTarget(D2D1::SizeF((FLOAT)uWidth, (FLOAT)uHeight), &m_pBitmapRenderTarget);
            }
